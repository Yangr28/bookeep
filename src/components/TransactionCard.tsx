  ...
  return (
    <div className="relative overflow-hidden rounded-card mb-2">
      <div className="absolute inset-0 flex items-center">
        <div className="absolute right-0 top-0 bottom-0 flex items-center justify-center w-16 bg-red-500 rounded-card">
        ...
        <div className="absolute left-0 top-0 bottom-0 flex items-center justify-center w-16 bg-primary-500 rounded-card">
      ...
      <div
        className="relative bg-white dark:bg-gray-800 p-3 shadow-card transition-transform duration-200 ease-out"
      ...
      >
        <div className="flex items-center gap-3">
          <div className="p-2.5 rounded-full flex-shrink-0" style={{ backgroundColor: category ? `${category.color}20` : '#f3f4f6' }}>
            <IconComponent size={20} style={{ color: category?.color || '#9ca3af' }} />
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between">
              <p className="font-medium text-gray-800 dark:text-white truncate">{category?.name || '未分类'}</p>
              <span className={`font-semibold flex-shrink-0 ml-2 truncate ${isIncome ? 'text-primary-600 dark:text-primary-400' : 'text-red-500 dark:text-red-400'}`}>
                ...
                {transaction.currency && transaction.currency !== 'CNY' && transaction.originalAmount !== undefined && (
                  <span className="text-xs text-gray-400 font-normal ml-1">({getCurrencySymbol(transaction.currency)}{transaction.originalAmount})</span>
                )}
              </span>
            </div>
            <div className="flex items-center gap-2 mt-1">
              {transaction.note && <p className="text-xs text-gray-400 truncate flex-1 max-w-[120px]">{transaction.note}</p>}
              <span className="text-xs text-gray-400 flex-shrink-0">{formatDateTime(transaction.createdAt)}</span>
            </div>
          </div>
        </div>
      </div>
